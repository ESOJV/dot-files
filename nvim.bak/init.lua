...
vim.keymap.set('n', 'd]', vim.diagnostic.goto_next, { desc = 'goes to previous diagnostic' })

-- Exit terminal
vim.keymap.set('t', '<Esc><Esc>', '<C-\\><C-n>', { desc = 'Exit terminal mode' })

-- NOTE: Pane movement handled by tmux, Ctrl+h/j/k/l used for harpoon
...
-- New horizontal shell pane (bottom)
...
-- TAB MOVEMENT
-- ============================================================================
-- Move tab right (next tab, wraps around)
vim.keymap.set('n', '<leader>tl', '<cmd>tabnext<CR>', { desc = '[T]ab [L]eft (next)' })
vim.keymap.set('n', '<leader><Tab>', '<cmd>tabnext<CR>', { desc = 'Next tab' })

-- Move tab left (previous tab, wraps around)
...
-- ============================================================================
-- Smart close: closes buffer if multiple exist, otherwise closes window/tab
vim.keymap.set('n', '<leader>x', function()
  local buffers = vim.fn.len(vim.fn.getbufinfo { buflisted = 1 })
  local tabs = vim.fn.tabpagenr '$'
  local is_terminal = vim.bo.buftype == 'terminal'

  -- Temporarily disable confirm to avoid prompts
  local old_confirm = vim.o.confirm
  vim.o.confirm = false

  if buffers > 1 then
    -- Multiple buffers: close current buffer (force for terminals)
    vim.cmd(is_terminal and 'bdelete!' or 'bdelete')
  elseif tabs > 1 then
    -- Multiple tabs but single buffer: close tab
    vim.cmd 'tabclose'
  else
    -- Single buffer and single tab: quit (force for terminals)
...
-- NOTE: Terminal pane movement removed — using tmux for pane navigation
...
require('lazy').setup({
  ...
  'prichrd/netrw.nvim', -- better looking net rw
    ...
    config = function()
      ...
      vim.api.nvim_create_autocmd('LspAttach', {
        ...
        callback = function(event)
          ...
          -- Fuzzy finds all the symbols in your current document.
  ...
  { -- Highlight, edit, and navigate code
    ...
    -- Install parsers with :TSInstall <language> (e.g., :TSInstall python)
  },
